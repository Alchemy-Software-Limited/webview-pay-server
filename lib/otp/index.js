...
const createOtp = async (payload) => {
	...
	const generateOtp = Math.round(Math.random() * 1000000)
		.toString()
		.padStart(6, "0");
...
const verifyOtp = async (payload) => {
	...
	if (foundOtp.otp !== payload.otp)
		throw new ApiError(httpStatus.CONFLICT, "OTP not matched");
	if (foundOtp.isVerified)
		throw new ApiError(httpStatus.CONFLICT, "OTP already verified");
